...
const App = () => {
    ...
    const [PagoKm, setPagoKm] = useState('');
                ...
                if (r.data.data.messageStatus == '1') {
                    ...
                    axios.get(Global.url + 'Transportista/Buscar?id=' + id)
                        .then((r) => {
                            ...
                            console.log(r)
                            setNombres(r.data.tran_Nombres);
                            setApellidos(r.data.tran_Apellidos);
                            setIdentidad(r.data.tran_Identidad);
                            setSexo(r.data.tran_Sexo);
                            setTelefono(r.data.tran_Telefono);
                            setPagoKm(r.data.tran_PagoKm)
                            setDirrecion(r.data.tran_DireccionExacta);

                            var fecha = new Date(r.data.tran_FechaNacimiento)
                            setFechaNac(fecha);
    ...
    const Editar = (e) => {
        ...
        if (!Nombres.trim()     || !Apellidos.trim()    || !Identidad.trim() || !FechaNac || !Sexo ||
            ...
            let Transportista = {
                tran_Id : parseInt(id),
                tran_Nombres: Nombres.trim(),
                tran_Apellidos: Apellidos.trim(),
                tran_Identidad: Identidad.trim(),
                tran_FechaNacimiento: FechaNac,
                tran_Sexo: Sexo,
                eciv_Id: EstadoCivil.code,
                muni_Id: Municipio.code,
                tran_DireccionExacta: Dirrecion.trim(),
                tran_Telefono: Telefono.trim(),
                tran_PagoKm : PagoKm,
                tran_UsuModificacion: parseInt(localStorage.getItem('usuID'))
            }

            axios.post(Global.url + 'Transportista/Editar', Transportista)
                .then((r) => {

                    if (r.data.data.messageStatus == '1') {
                        localStorage.setItem('Transportista', '2');
                        router.push('./Transportistas_index')
                    }
        ...
        <div className='col-12'>
            ...
            <div className="card" style={{ background: `rgb(105,101,235)`, height: '100px', width: '100%' }}>
                <div className="row text-center d-flex align-items-center">
                    <h2 style={{ color: 'white' }}>Ingresar Transportistas</h2>
                </div>
            </div>
            ...
            <div className="card">
                <div className="grid p-fluid">
                    ...
                    <div className='col-6'>
                        <div className="field">
                            <label htmlFor="Sexo">Pago Por Km (en Lempiras)</label><br />
                            <InputText
                                type="text"
                                id="inputtext"
                                value={PagoKm}
                                onChange={(e) => setPagoKm(e.target.value)}
                                onKeyPress={(event) => {
                                    if (!/[.0-9]/.test(event.key)) {
                                        event.preventDefault();
                                    }
                                }}
                                className={classNames({ 'p-invalid': submitted && !PagoKm })}
                            />
                            {submitted && !PagoKm && <small className="p-invalid" style={{ color: 'red' }}>El campo es requerido.</small>}
                        </div>
                    </div>
                    ...
                    <div className='col-6'>
                        <div className="grid p-fluid">
                            ...
                            <div className='col-6'>
                                <Button label="Cancelar" severity="default" onClick={() => router.push('./Transportistas_index')} />
                            </div>
